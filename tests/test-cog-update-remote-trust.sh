...
(
  ...
  git remote set-url cog-upstream "$trusted"
  ensure_remote >/dev/null

  if ! remote_urls_match \
    'git@github.com:huytieu/COG-second-brain.git' \
    'https://github.com/huytieu/COG-second-brain.git'; then
    echo "expected canonical SSH and HTTPS GitHub URLs to be treated as equivalent" >&2
    exit 1
  fi

  if remote_urls_match \
    'https://github.com.evil.example/huytieu/COG-second-brain.git' \
    'https://github.com/huytieu/COG-second-brain.git'; then
    echo "lookalike GitHub hostname was accepted" >&2
    exit 1
  fi

  if remote_urls_match \
    'https://github.com/huytieu/COG-second-brain.evil.git' \
    'https://github.com/huytieu/COG-second-brain.git'; then
    echo "lookalike repository path was accepted" >&2
    exit 1
  fi
)

echo "cog-update validates upstream remote identity before fetching"
